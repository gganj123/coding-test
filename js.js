function solution(n, info) {
  ...
  function dfs(index, apeachScore, lionScore, arrowsLeft) {
    if (index === 11) {
      ...
      if (difference > maxDifference) {
        ...
        while (
          ...
          bestAnswer[currentLowestIndex] === answer[currentLowestIndex]
        ) {
          currentLowestIndex--;
        }
    ...
    for (let lionArrows = 0; lionArrows <= arrowsLeft; lionArrows++) {
      ...
      dfs(
        index + 1,
        apeachScore + (apeachArrows > lionArrows ? 10 - index : 0),
        lionScore + (lionArrows > apeachArrows ? 10 - index : 0),
        arrowsLeft - lionArrows
      );
    }
  }
